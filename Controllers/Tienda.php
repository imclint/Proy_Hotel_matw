	...
	class Tienda extends Controllers{
		...
		public function producto($params){
			if(empty($params)){
				...
				$data['productos'] = $this->getProductosRandom($infoProducto['id_categoria'],8,"r");
		...
		public function addCarrito(){
			if($_POST){
				...
				// }else{
				// 	$arrResponse = array("status" => false, "msg" => 'Producto ya se encuentra Agregado');
				// }
				if(is_numeric($idproducto) and is_numeric($cantidad)){
					$arrInfoProducto = $this->getProductoIDT($idproducto);
					if(!empty($arrInfoProducto)){
						$arrProducto = array('idHabitacion' => $idproducto,
						...
						if(isset($_SESSION['arrCarrito'])){
							...
							for ($pr=0; $pr < count($arrCarrito); $pr++) {
								if($arrCarrito[$pr]['idHabitacion'] == $idproducto){
		...
		public function registro(){
			...
			if($_POST){
				if(empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmailCliente']))
				{
					...
					$strPassword =   hash("SHA256",$_POST['txtPassCliente']);
		...
		public function procesarVenta(){
			if($_POST){
				...
				$direccionenvio = strClean("*").' '.strClean($_POST['ciudad']);
				$status = "Pendiente";
				$subtotal = 0;
					...
					if(empty($_POST['datapay'])){
						...
						if($request_pedido > 0 ){
							...
							foreach ($_SESSION['arrCarrito'] as $producto) {
								$productoid = $producto['idHabitacion'];
						...
						if(is_object($objPaypal)){
							...
							if($objPaypal->status == "COMPLETED"){
								...
								// echo count($_SESSION['arrCarrito']);
								// die;
								$request_pedido = $this->insertPedido($idtransaccionpaypal, 
																	$datospaypal, 
																	$personaid,
								...
								if($request_pedido > 0 ){
									...
									foreach ($_SESSION['arrCarrito'] as $producto) {
										...
										$this->insertDetalle($request_pedido,$productoid,$precio,$cantidad);
										// $this->updateProd_reserv($productoid);
										// sleep(10);										
									}
