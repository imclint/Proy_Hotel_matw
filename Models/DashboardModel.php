<?php 
	class DashboardModel extends Mysql
	{
		...
		public function cantProductos(){
			$sql = "SELECT COUNT(*) as total FROM habitaciones WHERE status != 0 ";
			$request = $this->select($sql);
			$total = $request['total']; 
			return $total;
		}
		public function cantHabitacionesDisponibles(){
			$sql = "select COUNT(*) as total from habitaciones where STATUS =2 ";
			$request = $this->select($sql);
			$total = $request['total']; 
			return $total;
		}
